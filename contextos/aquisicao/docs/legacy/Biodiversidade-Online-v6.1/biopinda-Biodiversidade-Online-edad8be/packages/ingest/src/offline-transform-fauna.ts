...
import { processaFauna } from './fauna.ts'
import {
...
/**
 * Script para transformação offline de fauna
...
/**
 * Transforma dados de fauna offline usando dados preservados
 */
export async function transformFaunaOffline(
  mongoUri: string,
  ...
  try {
    ...
    for (let i = 0; i < originalDocuments.length; i += batchSize) {
      ...
      for (const [currentIptId, iptDocs] of iptGroups) {
        try {
          // Converter documentos para formato esperado pela função processaFauna
          const faunaJson: Record<string, any> = {}
          iptDocs.forEach((doc: DocumentoOriginal, index: number) => {
            faunaJson[`${doc.iptId}_${index}`] = doc.original_data
          })

          // Aplicar transformações
          const transformedData = processaFauna(faunaJson)
          ...
          if (!dryRun) {
            ...
            await preservador.markAsProcessed(processedIds, 'fauna')
          }
...
async function main() {
  ...
  console.log('Iniciando transformação offline de fauna...')
  console.log('Opções:', options)

  try {
    const stats = await transformFaunaOffline(mongoUri, options)
