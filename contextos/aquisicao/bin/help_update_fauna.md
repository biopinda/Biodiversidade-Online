...
Importa o catálogo de fauna brasileira de um endpoint IPT para a coleção `taxa` do MongoDB (`dwc2json`). Processa o arquivo DwC-A completo incluindo extensões (distribuição, nomes vernaculares, perfil de espécie, relações, referências, tipos).
...
| `IPT_FAUNA_URL`    | **sim**            | —          | URL do DwC-A de fauna (ex: `https://ipt.jbrj.gov.br/reflora/archive.do?r=fauna`)  |
...
.\bin\update-fauna.exe --dry-run

# Log detalhado
.\bin\update-fauna.exe --log-level debug

# .env em local alternativo
.\bin\update-fauna.exe --config C:\segredo\.env
