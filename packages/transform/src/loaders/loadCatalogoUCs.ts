...
const COLLECTION_NAME = 'catalogoucs'

async function main() {
  const csvPath = process.argv[2]
  if (!csvPath) {
    console.error('Uso: bun run load:catalogo-ucs -- <caminho-do-csv>')
    process.exit(1)
  }
  ...
  await collection.createIndex({ 'Nome da UC': 1 })
  await collection.createIndex({ UF: 1 })
