...
const COLLECTION_NAME = 'faunaAmeacada'

async function main() {
  const csvPath = process.argv[2]
  if (!csvPath) {
    console.error('Uso: bun run load:fauna-ameacada -- <caminho-do-csv>')
    process.exit(1)
  }
  ...
  await collection.createIndex({ canonicalName: 1 })
  await collection.createIndex({ threatStatus: 1 })
