...
2. **Given** developer expands endpoint in Swagger (e.g., GET /api/species), **When** views details, **Then** sees request parameters, response schema, and code examples
3. **Given** developer makes API call GET /api/species?threatStatus=endangered, **When** request completes, **Then** receives JSON array of threatened species with HTTP 200
