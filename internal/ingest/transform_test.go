...
func TestCoerceRecord_OmitsNullValues(t *testing.T) {
	...
	doc, _ := coerceRecord(record, taxaSchema)
...
func TestCoerceRecord_TypeCoercions(t *testing.T) {
	...
	doc, _ := coerceRecord(record, schema)
	...
	if _, ok := doc["locality"].(string); !ok {
		t.Errorf("locality should be string, got %T", doc["locality"])
	}
}

func TestCoerceRecord_DropsUnknownFields(t *testing.T) {
	record := map[string]string{
		"taxonID":        "1",
		"scientificName": "Species one",
		"someRandomCol":  "value",
	...
	doc, dropped := coerceRecord(record, taxaSchema)

	if dropped != 1 {
		t.Errorf("dropped = %d, want 1", dropped)
	}
	if _, ok := doc["someRandomCol"]; ok {
		t.Error("someRandomCol should have been dropped, it is not in the fixed schema")
	}
	if _, ok := doc["taxonID"]; !ok {
		t.Error("taxonID should be present")
	}
