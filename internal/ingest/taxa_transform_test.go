...
func TestNormalizeSpeciesProfile(t *testing.T) {
	...
	inner, ok := got["lifeForm"].(map[string]any)
	if !ok {
		t.Fatalf("lifeForm should be map[string]any, got %T", got["lifeForm"])
	}
	lifeForm, _ := inner["lifeForm"].([]string)
	if !reflect.DeepEqual(lifeForm, []string{"Árvore", "Arbusto"}) {
		t.Errorf("lifeForm = %v, want [Árvore Arbusto]", lifeForm)
	}
	habitat, _ := inner["habitat"].([]string)
	if !reflect.DeepEqual(habitat, []string{"Terrícola"}) {
		t.Errorf("habitat = %v, want [Terrícola]", habitat)
	}
}
